...
| 智谱 GLM API Key | 智谱开放平台 | LLM 推理（ReAct 决策 + Patch 生成） | ✅ 必须 |
...
| `glm_base_url` | `".../coding/paas/v4"` | **智谱专用端点**（非通用 `/api/paas/v4`，否则 429 错误） |
...
> **端点陷阱**：`glm_base_url` 必须使用 智谱专用端点 `https://open.bigmodel.cn/api/coding/paas/v4`，而非通用端点 `https://open.bigmodel.cn/api/paas/v4`。误用通用端点会导致 429/1113 错误。这个区别在注释中已标注。
...
Sources: [cli.py](../../src/eda_agent/cli.py#L73-L90)
...
> `runs/` 目录在 `.gitignore` 中被排除（`runs/*`），但 `runs/eval_snapshot/` 作为可复现证据被保留。
...
Sources: [cli.py](../../src/eda_agent/cli.py#L26-L27)
...
> **无 EDA 工具环境时**：`@pytest.mark.needs_eda` 自动 skip 真跑测试，`runs/eval_snapshot/` 提供替代证据。stub e2e 测试（`test_e2e_stub`）无 EDA 也能跑通，保证可行性验证不依赖外部工具。

Sources: [pyproject.toml](../../pyproject.toml), [conftest.py](../../tests/conftest.py#L1-L37)
...
| `429 / 1113` API 错误 | 使用了通用端点而非智谱专用端点 | 确认 `settings.toml` 中 `glm_base_url` 为 `.../coding/paas/v4` |
