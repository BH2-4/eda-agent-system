...
以 `runs/eval_snapshot/` 中的 `counter_bitwidth` 设计（Run `20260705_162902_3d2f`）为例，还原数据流中每一步的输入输出。该 Run 使用 rule 模式 + glm provider，共 5 个 planner iteration（其中第 4 步是 `skill_self_heal`，内部跑了 3 轮迭代），总耗时 61.7 秒。

**Steps 序列**（planner 共 5 个高层 iteration，`planner_iterations=5`；其中 `skill_self_heal` 步骤内部展开为 3 轮子迭代，见 report.md trajectory）：

| 阶段 | Tool | Skill | Iter | 语义 | 关键输出 |
|---|---|---|---|---|---|
| 基线综合 | `yosys_synth` | — | — | 初始综合 | `synth/netlist.v` + `synth/synth.json` |
| 基线仿真 | `iverilog_sim` | — | — | 初始仿真 | `passed=False`（bitwidth 错误，见 report iter0 `sim=False/0/0`） |
| 根因诊断 | `skill_diagnose` | — | — | 诊断根因 | 根因归因（bitwidth 类错误） |
| self-heal 迭代 | `yosys_synth`+`iverilog_sim`+`skill_diagnose` | `self_heal` | 0-2 | B 内部 3 轮迭代 | iter0: diff 失败; iter1: full_rewrite 通过; iter2: 验证通过 |
| 收敛返回 | `skill_self_heal` | — | — | B 返回最终结果 | `convergence=all_pass`, `best_iter=2`, `patch_source=llm_full_rewrite` |
| 独立验证 | `iverilog_sim` | — | — | C 独立验证 | `passed=True` → `goal_achieved=True` |

B 内部 trajectory 显示策略升级路径：iter 0 使用 `diff` 策略（LLM 产出 diff 但未成功应用，`applied=False`），iter 1 升级到 `full_rewrite` 策略（LLM 整文件重写成功，`applied=True`），iter 2 仿真通过并收敛（`sim=True/1/1`）。最终 `experiment_manifest.json` 记录 `self_heal_pass_rate=1.0`、`llm_calls=2`、`tokens_total=2492`、`wall_time_s=61.7`、`candidates_at_best_score=1`。

Sources: [experiment_manifest.json](../../runs/eval_snapshot/manifests/counter_bitwidth.json), [self_heal/report.md](../../runs/eval_snapshot/reports/counter_bitwidth.report.md), [fixed_rtl/counter_bitwidth.rtl.v](../../runs/eval_snapshot/fixed_rtl/counter_bitwidth.rtl.v)
