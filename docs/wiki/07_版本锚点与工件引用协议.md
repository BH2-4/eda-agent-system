...
这三条原则共同保证了系统在多组件协作（A 诊断器、B 自修复、C Planner）和多 LLM 后端（Claude/Qwen/DeepSeek）切换时的**契约一致性**与**工件可追溯性**——这正是完赛奖"实验记录"要求的底层技术保障。
...
- [实验聚合与通过率门槛（S1 达标规则）](28_实验聚合通过率门槛S1.md)：experiment_manifest 的聚合分析