本文档是 Agentic EDA Agent System 的入门起点。你将了解这个系统**是什么**、**为什么存在**、**核心架构如何分层**，以及**从哪里开始阅读后续文档**。无论你是新加入的开发者还是想快速理解全貌的读者，这里都是理解全貌的第一站。
...
本项目是一个 Agentic EDA Infra 系统，核心命题是：让智能体接入 EDA 工具链，把芯片设计流程变成可执行闭环——从设计生成、仿真验证、综合评估到错误分析和迭代优化，全程自动化。

系统围绕四个工程目标设计：**真实可跑的组件、清晰可调用的接口、可复现的实验证据、充分体现 Agentic 特性的设计**。每一条目标都有明确的落点和验收方式。
...
| ④ Agentic 充分 | C 默认 LLM ReAct planner + B 内部迭代 + patch 回退 + C 独立验证步 | 端到端 e2e 测试必过 |
...
| **LLM 主力** | 智谱 GLM-5.2 | OpenAI 兼容端点，思考模式 + max 推理 |
| **LLM 备用** | Anthropic Claude (claude-sonnet-4) | 经 provider 抽象可热切换 |
| **LLM 扩展** | Qwen-Plus / DeepSeek（可选） | OpenAICompatProvider 适配 |
...
系统已用智谱 **GLM-5.2** 完成全量 8 个注入 bug 的端到端实验，达到 S1 硬门槛达标线。
